...
async function saveRem() {
  ...
  if (editingRemId) {
    await _supabase.from('reminders').update({
      title: title, date: date, assigned: $('remFor').value
    }).eq('id', editingRemId);
    editingRemId = null;
  } else {
...
function openRemEdit(r) {
  ...
  $('remFor').value = r.for || r.assigned || 'both';
  var hdr = document.querySelector('#remForm p');
...
function renderReminders() {
  ...
  if (up.length) {
    ...
    up.forEach(function(r) {
      ...
      row.querySelector('.done-btn').addEventListener('click', function() { _supabase.from('reminders').update({ done: true }).eq('id', r.id); });
      row.querySelector('.del-btn').addEventListener('click', function() { _supabase.from('reminders').delete().eq('id', r.id); });
  ...
  if (done.length) {
    ...
    el.appendChild(hdr2);
      ...
      row.querySelector('.undone-btn').addEventListener('click', function() { _supabase.from('reminders').update({ done: false }).eq('id', r.id); });
      el.appendChild(row);
    });
  }
}
