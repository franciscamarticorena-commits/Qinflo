...
>
> **Actualización de una sesión posterior (mismo día)**: `CLAUDE_HANDOFF.md` fue **eliminado** poco después de escribirse este documento — tener dos archivos con "handoff" en el nombre (este y aquel) generó ambigüedad real: una sesión nueva, al pedírsele "leé el handoff", no tenía forma de saber a cuál se refería el usuario. Este archivo (`HANDOFF.md`) es ahora el único documento operativo con ese nombre. Ver sección 16, lección 7, para el detalle de cómo se detectó y corrigió.
...
| `CLAUDE_HANDOFF.md` | ~~Handoff operativo del turno anterior~~ — **eliminado**, su contenido quedó absorbido en este documento. Ver nota al inicio de este archivo. | — |
...
7. **Nombrar dos documentos con la misma palabra clave (`HANDOFF.md` y `CLAUDE_HANDOFF.md`) rompió la usabilidad real del set de documentación, no solo en teoría.** Ocurrió exactamente como se podía predecir: en una sesión nueva, el usuario pidió "leé el handoff" y la IA preguntó de vuelta "¿desde qué archivo?" — perdiendo el propósito completo de dejar la documentación lista para que una sesión nueva la usara sin fricción. La causa no fue un error de contenido (ambos archivos eran precisos) sino un error de **nomenclatura entre archivos**, algo que la sección "Cómo mantener la documentación sincronizada" de `AI_MEMORY.md` no cubría explícitamente antes de este incidente. **Fix aplicado**: se eliminó `CLAUDE_HANDOFF.md`, dejando `HANDOFF.md` como único documento operativo con ese nombre, y se corrigieron las ~7 referencias cruzadas a `CLAUDE_HANDOFF.md` en el resto de los documentos (`CLAUDE.md`, `README.md`, `PROJECT_STATUS.md`, `AI_MEMORY.md`, este mismo archivo, `CHANGELOG.md`). **Lección para el futuro, aplicable más allá de este caso**: antes de nombrar un archivo de documentación nuevo, `ls *.md` y confirmar que ningún archivo existente comparte una palabra clave lo bastante genérica como para que alguien, sin memoria de la sesión en que se creó, no pueda diferenciarlos con una instrucción corta tipo "leé el X". Un nombre de archivo es, en la práctica, parte de la interfaz de usuario de la documentación — no solo una etiqueta interna.
